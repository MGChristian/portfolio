  ...
  return (
    <>
      <nav className="sticky top-0 z-50 hidden h-20 items-center justify-between px-8 py-2 backdrop-blur-sm lg:flex">
        <div className="flex w-50 items-center justify-center gap-4 text-white -mr-10">
            <Link to="/"><img src={Logo} className="h-12 w-full" /></Link>
      ...
      <nav className="sticky top-0 z-50 flex h-20 items-center justify-between p-4 backdrop-blur-2xl lg:hidden">
        <div className="flex w-[200px] h-[80px] items-center justify-center text-white gap-4">
          <Link to="/"><img src={Logo} className="h-12 w-full" /></Link>
          <p className="unbounded font-normal text-shadow-black text-shadow-lg">Team <span className="text-sky-400">Synergy</span></p>
        </div>
        <div
      ...
      >
        {navValues.map((item, index) => {
          return (
            <div className={`group relative flex h-full w-full text-sky-400 items-center justify-center overflow-hidden rounded-full transition-transform duration-700 ease-in-out hover:scale-90 ${currentPage == item.src ? "scale-105 bg-neutral-800/75" : "scale-100"}`}>
              <Link
                key={index}
